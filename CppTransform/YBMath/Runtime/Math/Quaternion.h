...
inline Quaternionf AngleAxisToQuaternion(float degree, const Vector3f& axis)
{
	return AxisAngleToQuaternion(axis, Deg2Rad(degree));
}
...
inline
std::ostream& operator << (std::ostream& out, const Quaternionf& p)
{
	out << "[" << p.x << ", " << p.y << ", " << p.z << ", " << p.w << "]";
	return out;
}
