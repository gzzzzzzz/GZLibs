...
// Unaligned store, store vector at address base as 4 float
#define Vstoreupf(v, base, offset) memcpy( (base)+(offset), &v.m128_f32[0], sizeof(v))
...
// Unaligned load, load 4 float at address v in vector register
static MECANIM_FORCE_INLINE vec4f Vloadupf(float const* v, int offset)
{
	float const* p = v+offset;
	return vec4f( p[0], p[1], p[2], p[3] );
}
