...
class Vector3f
{
	...
	EXPORT_COREMODULE static const Vector3f	right;
	EXPORT_COREMODULE static const Vector3f	up;
	EXPORT_COREMODULE static const Vector3f	forward;
};
...
inline
std::ostream& operator << (std::ostream& out, const Vector3f& p)
{
	out << "[" << p.x << ", " << p.y << ", " << p.z << "]";
	return out;
}
